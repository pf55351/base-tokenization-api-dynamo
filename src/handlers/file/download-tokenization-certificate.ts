...
const dowloadTokenizationCertificateRoute = async (
  ...
  try {
    ...
    const res = await downloadCertificate(fileUUID);

    return {
      statusCode: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="${res.fileName}-certification"`,
...
export const handler = middy()
  ...
  .handler(dowloadTokenizationCertificateRoute)
